            ...
            <main class="admin-main">
                <div class="container card mt-5">
                    <form class="card-body" method="POST" x-data="{
                               price: ${pkg.getListPriceVND()},
                               percentage: ${pkg.getSalePercent()},
                               get salePrice() {
                        ...
                        <div class="form-container">
                            ...
                            <input pattern="\S(.*\S)?" type="text" class="form-control" name="name" id="name" value="${pkg.getPackageName()}" required placeholder="Enter package name" title="Input must not contain trailing space">

                            <i class="bi bi-clock"></i>
                            <label for="accessDuration" class="col-form-label">Access Duration (in months)</label>
                            <input type="number" value="${pkg.getDuration()}" min="1" max="48" name="duration" class="form-control" id="accessDuration" placeholder="Enter access duration in months" required>
                            
                            <i class="bi bi-info-circle"></i>
                            <label for="status" class="col-form-label">Status</label>
                            <input type="text" value="${pkg.isActive() ? 'Active' : 'Inactive'}" class="form-control" id="status" placeholder="Status" disabled>
